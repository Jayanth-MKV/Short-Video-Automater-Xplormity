...
def tts(text: str, voice: str, output_filename: str = "YT/data/output.mp3", play_sound: bool = False) -> None:
...
def split_string(string: str) -> list[str]:
    words = [chunk.strip()+". " for chunk in string.split('|')]
    result = []
    current_chunk = ""
    for word in words:
        if (
            len(current_chunk) + len(word) + 1 <= 300
